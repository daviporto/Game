...
public class Level extends Layer{
	...
	public boolean mobCollision(Mob m, int x, int y) {
		...
		while(i.hasNext()) {
			current = i.next();
			if(!(current instanceof Mob))continue;
	...
	public List<Entity> getEntities(int ex, int ey,  int radius){
		List<Entity> result = new  ArrayList<Entity>();
			...
			for(int i = 0; i < entities.size(); i++) {
				Entity entity = entities.get(i);
				if(entity.getX() == ex && entity.getY() == ey ) continue;
				if (entity instanceof Projectile) continue;
				if (entity instanceof ParticleSpawner) continue;
				int x = entity.getX();
				int y = entity.getY();
	...
	private void remove() {
		...
		for (int i = 0; i < players.size(); i++) {
			if(player.isRemoved()) players.remove(i);
	...
	public boolean projectileCollision(int x, int y,  int size, int xoffset, int yoffset,
										int damage, List<Entity> e, KindofProjectile kind) {
...
//		screen.renderSprite(100, 100, SpriteSheet.laser_green_sprites.getSprite()[0], false);
	...
	public Tile getTile(int x, int y) {
		...
		if(tiles[x + y * width] == Tile.colour_spawn_water)	return Tile.spawn_water;
		if(tiles[x + y * width] == Tile.colour_spawn_blue_rock )	return Tile.spawn_bluerock;
		if(tiles[x + y * width] == Tile.colour_spawn_bricks )	return Tile.spawn_brickss;
		if(tiles[x + y * width] == Tile.colour_spawn_grass)	return Tile.spawn_grass;
		if(tiles[x + y * width] == Tile.colour_spawn_purple_rock)	return Tile.spawn_purplerock;
		if(tiles[x + y * width] == Tile.colour_spawn_rock )	return Tile.spawn_rock;
		if(tiles[x + y * width] == Tile.colour_spawn_water )	return Tile.spawn_water;
		
		
		
		if(tiles[x + y * width] == Tile.colour_level1_floor)		return Tile.level1_floor;
		if(tiles[x + y * width] == Tile.colour_level1_wall1)		return Tile.level1_wall1;
		if(tiles[x + y * width] == Tile.colour_level1_wall2)		return Tile.level1_wall2;
		if(tiles[x + y * width] == Tile.colour_level1_wall3)		return Tile.level1_wall3;
		if(tiles[x + y * width] == Tile.colour_level1_wall4)		return Tile.level1_wall4;
		if(tiles[x + y * width] == Tile.colour_level1_wall_back)	return Tile.level1_wall_back;
		if(tiles[x + y * width] == Tile.colour_level1_left)			return Tile.level1_wall_left;
		if(tiles[x + y * width] == Tile.colour_level1_wall_right)	return Tile.level1_wall1_right;
		if(tiles[x + y * width] == Tile.colour_level1_left_corner)	return Tile.level1_left_corner;
		if(tiles[x + y * width] == Tile.colour_level1_right_corner)	return Tile.level1_right_corner;
		if(tiles[x + y * width] == Tile.colour_level1_upper_corner)	return Tile.level1_upper_corner;
