...
public class NivelDeTeste extends Level{
	...
	protected void loadLevel(String path) {
		...
		UITextandNext basicatkPanel = new UITextandNext(new Vector2i(150, 20),
		...
		UITextandNext movingPanel = new UITextandNext(new Vector2i(150,20),
				new Vector2i(500, 100),
				"como andar", 10);
		movingPanel.addLine("para se mover use W A S D ou as setas, voce pode atirar"
				+ " enquanto se move" );
		
		ui.addTextPanel(basicatkPanel);
		ui.addTextPanel(movingPanel);
		
		UITextandNext proximaSala = new UITextandNext(new Vector2i(150, 20),
				new Vector2i(500, 100),
				 "primeiros passos", 10);
		proximaSala.addLine("ande em direção a sala a sua frente");
		ui.addTextPanel(proximaSala);
		
		chegadoSala1 = new UITextandNext(UITextandNext.defoultTextPosition, UITextandNext.defoultTextsize,
										"aaaa", 20);
		chegadoSala1.setResizable(true);
		String text = "estes monstros ao seu redor sao pacificos, eles não revidam ao "
				+ "ao serem atacados, apenas correm. Mate-os para ganhar experiencia";
		chegadoSala1.addLine(text);
...
//		}
	...
	public void addLocationTrigerredEvents() {
		messagesManager.AddEvent(new LocationTrigered(new Vector2i(304, 351),
									new Vector2i(410, 451),	chegadoSala1));
		
		messagesManager.AddEvent(new LocationTrigered(new Vector2i(500, 600),
				new Vector2i(576, 620),	new UITextandNext(UITextandNext.defoultTextPosition,
						UITextandNext.defoultTextsize,"fantasmas", "esses fantasmas vagam pelo mapa aleatoriamente,"
								+ "eles não te perseguem mas caso voce entre no range deles, esse irão te atacar", 10)));
		
		messagesManager.AddEvent(new LocationTrigered(new Vector2i(950, 1000),
				new Vector2i(528, 851),	new UITextandNext(UITextandNext.defoultTextPosition,
						UITextandNext.defoultTextsize,"cabeças de abobora", "essa cabeças de abobora são semelhantes aos fantas"
								+ "mas so te atacarão caso entre no range deles", 10)));
		

		messagesManager.AddEvent(new LocationTrigered(new Vector2i(1238, 1283),
				new Vector2i(496, 783),	new UITextandNext(UITextandNext.defoultTextPosition,
						UITextandNext.defoultTextsize,"bruxas", "as bruxas são imunes a veneno"
								+ "e podem te envenenar cuidado!!!, elas diferente dos outros inimigos podem te perseguir"
								+ "sendo capaz de desviar ate mesmo de paredes", 10)));
		
		messagesManager.AddEvent(new LocationTrigered(new Vector2i(1803, 1875),new Vector2i(784, 834),
				new UITextandNext("magos trigemeos", "a frente voce encontrara 3 magos, cada um com um tipo de abilidade"
