...
public class AnimatedFireboolProjectile extends Projectile {
	protected final Random random = new Random();
    private AnimatedSprite anim;
	public static final int FIRE_RATE = 60; //higher is slower
	public static final KindofProjectile type = KindofProjectile.FIREBOOL;

	public AnimatedFireboolProjectile(double x, double y, double direction, boolean player) {
		super(x, y, direction, player);
		range = random.nextInt(100) + 150;
		speed = 4;
		damage = 20;
		
		anim = new AnimatedSprite(SpriteSheet.firebools_projecitles_sprites, 22, 22, 10);
	...
	public AnimatedFireboolProjectile(double x, double y, double direction) {
	...
	public void update () {
		...
		if(level.tileCollision((int)(x + nx), (int)(y + ny ), 9,34, 0)) { 
			level.add(new ParticleSpawner ((int)x, (int)y + 16 , 30, 25, level, Sprite.FireParticles));
			remove();
