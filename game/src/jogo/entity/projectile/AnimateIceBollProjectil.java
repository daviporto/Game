...
public class AnimateIceBollProjectil extends Projectile{
	protected final Random random = new Random();
    private AnimatedSprite anim;
	public static final int FIRE_RATE = 60; //higher is slower
	public static final KindofProjectile type = KindofProjectile.ICEBOOL;

	public AnimateIceBollProjectil(double x, double y, double direction, boolean player) {
		super(x, y, direction, player);
		range = random.nextInt(100) + 150;
		speed = 5;
		damage = 25;
		
		anim = new AnimatedSprite(SpriteSheet.icebools_projecitles_sprites, 23, 23, 10);
		
		nx = speed * Math.cos(angle);
		ny = speed * Math.sin(angle);
	}
	
	public AnimateIceBollProjectil(double x, double y, double direction) {
		this(x, y, direction, true);
	}

	public void update () {
		anim.update();
		if(level.tileCollision((int)(x + nx), (int)(y + ny ), 9,0, 0)) { 
			level.add(new ParticleSpawner ((int)x, (int)y + 16 , 30, 25, level, Sprite.iceParticles));
			remove();
